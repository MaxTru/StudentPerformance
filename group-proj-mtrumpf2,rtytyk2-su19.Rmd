...
It would get a pretty high accuracy score of ``r accuracy(baseline_mod)``, which we wil try to beat.
...
The small additive model has better test accuarcy as well as $LOOCV$ RMSE. Both model are above the accuracy baseline of ``r accuracy(baseline_mod)``. 

So we will continue our search based on the small additive model

Now let's start searching for a better model using `AIC`:

```{r}
scope = formula(lm(G3 ~ (school + sex + studytime + schoolsup + higher) ^ 3, data = student_trn))
...
It seems quite hopeless to find a good model using only selected variables. Let's try putting G2,G1 and failures back in the model and do an analysis based on p-values. We've reached this interaction model:

```{r}
data = student_trn
data$outcome = as.factor(ifelse(data$G3 > 10, 1, 0))

int_mod = lm(G3 ~ (G2 + G1 + sex + age + absences + failures + famsize + paid + freetime + health) ^ 2 - 
               G2:failures - paid - G2:absences - G1:age - failures:freetime - famsize:paid - age - absences:paid, 
             data = student_trn)
```
...
```{r} 
student_trn$outcome = as.factor(ifelse(student_trn$G3 > 10, 1, 0))
student_tst$outcome = as.factor(ifelse(student_tst$G3 > 10, 1, 0))

bern_full = glm(outcome ~ . - G3, data = data, family = "binomial")
