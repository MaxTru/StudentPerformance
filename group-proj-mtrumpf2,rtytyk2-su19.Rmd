...
We reject null hypothesis. The more complex model performs better.

However, we also need to check how the models perform on unseen data:

```{r}
c(accuracy(add_mod2), accuracy(add_mod3))
c(loocv_rmse(add_mod2), loocv_rmse(add_mod3))
```

The simpler model performs slightly better on unseen data. However, there is no big delta. So we will at first continue with our full model.
...
selected_frw_full = step(add_mod3, scope = scope, direction = "forward", trace = 0)
coef(selected_frw_full)
```
...
n = nrow(student_trn)
scope = formula(lm(G3 ~ (. - G2 - G1) ^ 2, data = student_trn))
selected_frw_full_bic = step(add_mod3, scope = scope, direction = "forward", trace = 0, k = log(n))
...
Since the high amount of VIFs > 5 is suspicious, let's go back to our similarly well performing smaller model and see how this can be improved:
