...
rmse_train = function(model) {
  predicted = predict(model, student_trn)
  sqrt(mean((student_trn$G3 - predicted) ^ 2))
}

rmse_test = function(model) {
  predicted = predict(model, student_tst)
  sqrt(mean((student_tst$G3 - predicted) ^ 2))
}
...
Now we'll build full additive model using and compare P-values to $\alpha=0.05$:
...
```{r}
add_mod2 = lm(G3 ~ . - G2 - G1, data = student_trn)
influence(add_mod2)
```

We're discovering `school`, `failures`, `higher`, `health` are being the most influential predictors, rejecting null hypothesis for any reasonable $\alpha$ level. Let's build a model out of those predictors:

```{r}
add_mod3 = lm(G3 ~ school + failures + higher + health, data = student_trn)
```

And compare it to the full additive model that has `G2` and `G1` removed:

```{r}
anova(add_mod3, add_mod2)$`Pr(>F)`[2]
```

We reject null hypothesis. There are other predictors that we should consider and we'll use our third model to start searching for improvement.

Now let's see how our model does on the test set:
...
$81\%$ is a good start, now let's start searching for a better model using `AIC`:

```{r}
scope = formula(lm(G3 ~ (. - G2 - G1) ^ 2, data = student_trn))
selected_frw = step(add_mod3, scope = scope, direction = "forward", trace = 0)
coef(selected_frw)
```

```{r}
accuracy(selected_frw)
rmse_train(selected_frw)
rmse_test(selected_frw)
```

That's minimal improvement and we've added a lot of interactions. Let's look at VIFs bigger than 5:

```{r}
car::vif(selected_frw) > 5
```
