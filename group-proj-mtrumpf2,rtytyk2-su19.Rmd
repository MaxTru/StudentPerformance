...
There *383 females* and *266 males* in the dataset. A strong majority of 457 has more than 2 siblings. Most of the student aren't in romantic relationship and *strong majority of 580 want higher education*. On average our students report *good health* and *good relation* with their families. Strong majority of 455 have *mother* as their guardian. They go out on average 3 days a week, which is as much as they have free time. They are absent 3.66 days on average with one person being absent 32 days.

```{r}
student[student$absences > 20, c("G3", "absences", "sex", "age")]
```


#### Health

Let's first visualize how healthy are our students:

```{r, echo=FALSE}
hist(student$health, col = "dodgerblue", breaks=5, main = "Histogram of health", xlab = "health")
```

There are `r mean(student$health > 2)*100.0`% students that report health +3 and `r mean(student$health < 3)*100.0`% which are below 3.
...
Strangely, we're observing inverse relationship between `health` and `G3`. Seems like feeling good takes students off their studying.
...
Again, the females are doing better on average than males. As expected, we're seeing positive relationship between `studytime` and `G3` result, and inverse relationship between `absences` and `G3`.

#### Alkohol consumption and Activities

Now we'll look how alkohol consumption affect `G3` performance. We'll divide the plots into people who take off school `acivities` and those who do not.
