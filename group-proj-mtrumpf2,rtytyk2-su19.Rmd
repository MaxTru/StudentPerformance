...
From a visual inspection, the response could either follow a normal (green curve) or a Poisson distribution (orange points). Since there is not a large delta of the "fit", we will primarily be using **Linear Regression Models** over **Generalized Linear Models with link function for Poisson distribution**.
...
## Model interpretation {#discussionsChap}
...
Having data about the previously mentioned attriutes, makes the remaining predictors not significant for the prediction of student performance (`G3`). It should be highlighted, that factors which are often stated as being significant in the publich opinion, such as neighborhood (`address`) and family size (`famsize`).
